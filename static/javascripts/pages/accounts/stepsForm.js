...
;( function( window ) {
	...
	stepsForm.prototype._nextQuestion = function() {
		...
		if( !this.isFilled ) {
			...
			var text = $(this.el).find('li.current label').text();
			if (text.indexOf('q1') > -1) {
				var newText = text.replace('q1', $(this.el).find('[name="full_name"]').val().split(' ')[0]);
				$(this.el).find('li.current label').text(newText);
			}
